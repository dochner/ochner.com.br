<template>
  <header
    class="fixed top-0 inset-x-0 z-50 h-$header-height px-6 bg-white dark:bg-dark-800 !bg-opacity-80 backdrop-blur"
  >
    <div class="max-w-screen-lg mx-auto h-full flex justify-center flex-col md:flex-row md:justify-between items-center">
      <router-link class="text-2xl font-bold select-none outline-none" to="/" focusable="false">
        <h1 class="text-dark-800 dark:text-light-800">
          Douglas Ochner
        </h1>
      </router-link>
      <nav class="nav pt-2">
        <router-link to="/posts" title="Posts">
          <span class="lt-md:hidden">Blog</span>
          <div i-ri-article-line class="md:hidden" />
        </router-link>
        <router-link to="/bookmarks" title="Bookmarks">
          <span class="lt-md:hidden">Bookmarks</span>
          <div i-ri-bookmark-line class="md:hidden" />
        </router-link>
        <a href="https://www.linkedin.com/in/douglasochner/" target="_blank" rel="noopener" title="Linkedin">
          <div i-feather-linkedin />
        </a>
        <a href="https://github.com/dochner" target="_blank" rel="noopener" title="GitHub">
          <div i-uil-github-alt />
        </a>
        <a href="/feed.xml" target="_blank" title="RSS" class="lt-md:hidden">
          <div i-la-rss-square style="font-size:1.25rem;" />
        </a>
        <ToggleTheme />
        <ToggleLocale />
      </nav>
    </div>
  </header>
</template>

<style scoped>
.nav {
  display: flex;
  align-items: center;
  gap: 0.2rem;
}
.nav img {
  margin-bottom: 0;
}
.nav a {
  cursor: pointer;
  text-decoration: none;
  color: inherit;
  transition: opacity 0.2s ease;
  opacity: 0.7;
  outline: none;
  padding: 0.5rem;
}
.nav a:hover {
  opacity: 1;
  text-decoration-color: inherit;
}
.backdrop-blur {
  backdrop-filter: blur(8px);
}
</style>
