...
import { formatDate } from '@/composables/use-format-date'
...
useHead(computed<HeadObject>(() => {
  ...
  return {
    title: `${head.title ? `${head.title} | ` : ''}Douglas Ochner`,
    meta,
    htmlAttrs: {
      lang: head.lang || 'en',
    },
  ...
  <div :class="{ 'post-list': isPostList }">
    <div v-if="frontmatter.title" class="max-w-$md-max-width m-auto mb-8">
      ...
      <div class="flex items-center">
        ...
        <ListTags v-if="route.path === '/posts'" />
      </div>
      <BtnShareGroup />
    </div>
    ...
    <div v-if="frontmatter.tags" class="max-w-$md-max-width m-auto mt-12">
      ...
      <div class="flex flex-wrap items-center text-light-blue-500 -mx-1">
        <BadgeTag v-for="tag in frontmatter.tags" :key="tag" class="m-1" @click="searchTag(tag)">
          <span>{{ tag }}</span>
        </BadgeTag>
      </div>
    </div>
